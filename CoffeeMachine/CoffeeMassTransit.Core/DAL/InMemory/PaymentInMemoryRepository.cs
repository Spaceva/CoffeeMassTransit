...
namespace CoffeeMassTransit.Core.DAL
{
    ...
    {
        private readonly Dictionary<Guid, List<Payment>> PaymentsByOrderId = new Dictionary<Guid, List<Payment>>();
        private readonly Dictionary<Guid, Payment> ActivePaymentByOrderId = new Dictionary<Guid, Payment>();
        ...
        public IReadOnlyCollection<Payment> Get(Guid orderId)
        {
            if (!PaymentsByOrderId.TryGetValue(orderId, out List<Payment> payment))
                throw new System.ArgumentOutOfRangeException(nameof(orderId));
